...
/**
 * @title UserKycThresholdToRule
 * @dev UserKycThresholdToRule checks if receiver KYC of a transfer is above a specific KYC threshold 
...
contract UserKycThresholdToRule is Initializable, AbstractRule {
  ...
  uint256 internal constant REASON_KYC_LESS_THAN_THRESHOLD = 2;
  ...
  */
  ...
  /**
  * @dev Validates a transfer if the from address KYC level is above the defined threshold
  ...
  * @return transferStatus Invalid transfer if KYC level is below the threshold, valid transfer without further action otherwise
  ...
  */
    ...
    public view returns (uint256, uint256)
  {
    ...
    if (complianceRegistry.attribute(trustedIntermediary, userId, USER_KYC_KEY) < _kycThreshold) {
      return (TRANSFER_INVALID, REASON_KYC_LESS_THAN_THRESHOLD);
    }
