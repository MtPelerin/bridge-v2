...
function isTransferValid(var self: state;  const from_ : address; const to_: address; const amountInRefCurrency_: nat; const realm_: address; const trustedIntermediaries_: list(address); const noCheckThreshold_ : nat; const callback_: contract(isTransferValidResult)) : (list(operation) * state) is
  block {
    self.noCheckThreshold := noCheckThreshold_;
    self.amountInRefCurrency := amountInRefCurrency_;
    var ops : list (operation) := list [];
    if from_ = self.complianceRegistry or to_ = self.complianceRegistry then block {
      const op : operation = Tezos.transaction(record [valid_ = transfer_valid_with_no_hook; reason_ = reason_ok ], 0tez, callback_);
      ops := op # ops;
    } else block {
...
function beforeTransferHook(const self: state; const token_: address; const from_: address; const to_: address; const amount_: nat; const trustedIntermediaries_: list(address); const callback_: contract(beforeTransferHookResult)) : list(operation) is
  block {
    failIfNotOwnerOrOperator(self);
    const complianceRegistryCall_ : contract(addOnHoldTransferParam) = case (get_entrypoint_opt("%addOnHoldTransfer", self.complianceRegistry) : option(contract(addOnHoldTransferParam))) of | None -> failwith("RU02") | Some(x) -> x end;
    const addOnHoldTransferParam_ : addOnHoldTransferParam = record [
      amount_ = amount_;
      from_ = from_;
      to_ = to_;
      token_ = token_;
    ...
    const opCompliance : operation = Tezos.transaction(addOnHoldTransferParam_, 0tez, complianceRegistryCall_);
    const opCallback: operation = Tezos.transaction(record [valid_ = transfer_valid_with_after_hook; amount_ = amount_; to_ = self.complianceRegistry ], 0tez, callback_);
    const ops : list (operation) = list [opCompliance; opCallback];
  } with (ops);
...
| IsTransferValid(param) -> isTransferValid(self, param.from_, param.to_, param.amountInRefCurrency_, param.realm_, param.trustedIntermediaries_, param.ruleParam_, param.callback_)
| ComplianceRegistryCallback(param) -> (complianceRegistryCallback(self, param), self)
| BeforeTransferHook(param) -> (beforeTransferHook(self, param.token_, param.from_, param.to_, param.amount_, param.trustedIntermediaries_, param.callback_), self)
