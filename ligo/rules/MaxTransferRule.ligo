...
const reason_amount_exceeds_max_amount = 1;
...
function isTransferValid(const amount_ : nat; const maxAmount_ : nat; const callback_ : contract(isTransferValidResult)) : list(operation) is
  block {
    var result : isTransferValidResult := record [
      valid_ = transfer_invalid;
      reason_ = reason_amount_exceeds_max_amount;
    ];
    if amount_ <= maxAmount_ then 
      block {
        result := record [
