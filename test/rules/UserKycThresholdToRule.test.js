...
const Contract = Contracts.getFromLocal('UserKycThresholdToRule');
...
contract('UserKycThresholdToRule', function ([_, tokenOwner, owner, trustedIntermediary1, trustedIntermediary2, address1, address2, address3, address4, address5]) {
  ...
  context('Check transfer validity', function () {
    it('returns that transfer is valid if kyc level is more than kycThreshold', async function () {
      const ret = await this.contract.methods.isTransferValid(token, address3, address1, 20000, 0).call();
    ...
    it('returns that transfer is valid if kyc level is equal to kycThreshold', async function () {
      const ret = await this.contract.methods.isTransferValid(token, address3, address1, 15000, 1).call();
      ret['0'].should.equal('1');
      ret['1'].should.equal('0');
    });
    it('returns that transfer is invalid if kyc level is less than kycThreshold', async function () {
      const ret = await this.contract.methods.isTransferValid(token, address3, address1, 10000, 2).call();
