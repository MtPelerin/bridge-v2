...
const Contract = Contracts.getFromLocal('YesNoUpdateRule');

contract('YesNoUpdateRule', function ([_, token, address1, address2]) {
  ...
  context('Update after transfer', function () {
    it('returns true to indicate that a state change has been done', async function () {
      (await this.contract.methods.updateCount().call()).should.equal('0');
      await this.contract.methods.afterTransferHook(token, address1, address2, 10000, 15000).send({from: address1});
      (await this.contract.methods.updateCount().call()).should.equal('1');
    });
  });
});