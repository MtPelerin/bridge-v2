  ...
  context('When operator', function () {
    ...
    context('Open resolution for vote', function () {
      ...
      it('can open resolution for vote', async function () {
        ({ events: this.events } = await this.contract.methods.openResolutionVote(0, 300).send({from: operator}));
      });

      it('emits ResolutionOpen event', function () {
        this.events.should.have.property('ResolutionOpen');
        this.events.ResolutionOpen.returnValues.should.have.property('votingEnd');
      });

      it('reverts if trying to open resolution that does not exist', async function () {
        await shouldFail.reverting.withMessage(this.contract.methods.openResolutionVote(3, 300).send({from: operator}), "VO07");
      });
    });  
    ...
    context('When voting started', function () {
      beforeEach(async function () {
        ({ events: this.events } = await this.contract.methods.addResolutions([bytes32('QUESTION1'), bytes32('QUESTION2')], [bytes32('https://mtpelerin.com/v1/q1'), bytes32('https://mtpelerin.com/v1/q2')], [2, 3]).send({from: operator, gas: 500000}));
        await this.contract.methods.openResolutionVote(0, 300).send({from: operator});
      });
  ...
  context('When standard user', function () {
    ...
    context('Resolutions', function () {
      ...
      it('reverts if trying to open resolution for voting when not authorized', async function () {
        await shouldFail.reverting.withMessage(this.contract.methods.openResolutionVote(0, 300).send({from: address1}), "OP01");
      });
    });
    ...
    context('Voters', function () {
      ...
      context('When voting started', function () {
        beforeEach(async function () {
          await this.contract.methods.openResolutionVote(0, 5).send({from: operator});
        });
        ...
        context('Multiple resolutions', function () {
          beforeEach(async function () {
            await this.contract.methods.openResolutionVote(1, 300).send({from: operator});
          });
      ...
      context('When voting ended', function () {
        beforeEach(async function () {
          await this.contract.methods.openResolutionVote(0, 1).send({from: operator});
          await timeout(2000);
        });
