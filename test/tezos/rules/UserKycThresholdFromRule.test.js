...
contract('UserKycThresholdFromRule', function ([owner, token, trustedIntermediary1, trustedIntermediary2, address1, address2, address3, address4]) {
  beforeEach(async function () {
    ...
    this.contract = await ContractBuilder.new(tezos, 'ligo/rules/UserKycThresholdFromRule.ligo', {}, {
  ...
  context('Check transfer validity', function () {
    ...
    it('returns that transfer is invalid if kyc level is less than kycThreshold', async function () {
      await runOperation(tezos, owner, () => this.contract.methods.isTransferValid(10000, 10000, this.callback.address, address1.pkh, BURN_ADDRESS, 2, address3.pkh, token.pkh, [trustedIntermediary1.pkh, trustedIntermediary2.pkh]).send());
      const result = await this.ruleEngine.storage();
